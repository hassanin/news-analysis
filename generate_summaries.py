...
async def summarize_article(article: Article, semaphore: asyncio.Semaphore):
    ...
    article_errors_file = "./data/article_errors.txt"
    async with semaphore:
        try:
            # summary = await chatbot.add_chatbot_message_async(prompt)
            # summary = chatbot.add_chatbot_message(prompt)
            summary = await sa(article.content)
            with open(
                f"./data/article_summaries/{article.id}.json", "w", encoding="utf8"
            ) as f:
                f.write(summary)
            global count
            count += 1
...
def insert_data_into_db():
    ...
    data_dir = "data/article_summaries/"
    ...
    for file_name in tqdm(os.listdir(data_dir)):
        ...
        update_article_summary(article_id=article_id, summary=article_summary)
        # break
